...
class WebTasksPage extends StatefulWidget {
  ...
  @override
  State<WebTasksPage> createState() => WebTasksPageStateInternal();
}

class WebTasksPageStateInternal extends State<WebTasksPage> {
  TasksTab tabInternal = TasksTab.queue;
  final TextEditingController searchControllerInternal =
      TextEditingController();
  String queryInternal = '';
  String? selectedTaskIdInternal;
  ...
  Widget build(BuildContext context) {
    return AnimatedBuilder(
      ...
      builder: (context, _) {
        ...
        final allItems = controller.taskItemsForTab(tabInternal.label);
        final items = allItems
            .where(matchesQueryInternal)
            .toList(growable: false);
        final selected = resolveSelectedTaskInternal(items);
        final metrics = [
          MetricSummary(
            ...
            child: Column(
              children: [
                SectionTabs(
                  ...
                  onChanged: (value) {
                    setState(() {
                      tabInternal = TasksTab.values.firstWhere(
                        (item) => item.label == value,
                      );
                      selectedTaskIdInternal = null;
                    });
                  },
                ),
                    ...
                    child: ClipRRect(
                      ...
                      child: Row(
                        children: [
                          SizedBox(
                            width: 360,
                            child: TaskListPanelInternal(
                              tab: tabInternal,
                              items: items,
                              selectedTaskId: selected?.id,
                              onSelectTask: (task) {
                                setState(
                                  () => selectedTaskIdInternal = task.id,
                                );
                              },
                            ),
                          ),
  ...
  bool matchesQueryInternal(DerivedTaskItem item) {
  ...
  DerivedTaskItem? resolveSelectedTaskInternal(List<DerivedTaskItem> items) {
    if (items.isEmpty) {
      return null;
    }
    for (final item in items) {
      if (item.id == selectedTaskIdInternal) {
        return item;
      }
    }
    return items.first;
...
class TaskListPanelInternal extends StatelessWidget {
  const TaskListPanelInternal({
    super.key,
    required this.tab,
    required this.items,
...
class TaskListTileInternal extends StatelessWidget {
  const TaskListTileInternal({
    super.key,
    required this.task,
            ...
            children: [
              Row(
                ...
                children: [
                  ...
                  StatusBadge(status: taskStatusInfoInternal(task.status)),
                ],
              ),
              ...
              Wrap(
                ...
                runSpacing: 6,
                children: [
                  InlineMetaInternal(label: task.owner),
                  InlineMetaInternal(label: task.startedAtLabel),
                  InlineMetaInternal(label: task.surface),
                ],
              ),
            ],
          ),
...
class TaskDetailPanelInternal extends StatelessWidget {
  const TaskDetailPanelInternal({
    super.key,
        ...
        children: [
          Wrap(
            ...
            children: [
              ...
              StatusBadge(status: taskStatusInfoInternal(selected!.status)),
            ],
          ),
          const SizedBox(height: 8),
          Text(
...
class DetailStatInternal extends StatelessWidget {
  const DetailStatInternal({
    super.key,
    required this.label,
    required this.value,
  });
...
class InlineMetaInternal extends StatelessWidget {
  const InlineMetaInternal({super.key, required this.label});
...
StatusInfo taskStatusInfoInternal(String status) => switch (status) {
  'running' ||
