...
class AssistantSidePaneInternal extends StatelessWidget {
  const AssistantSidePaneInternal({
    super.key,
    required this.collapsed,
  ...
  final bool collapsed;
  final WebAssistantPaneInternal activePane;
  final AppController controller;
  final String query;
  ...
  final VoidCallback onToggleCollapsed;
  final ValueChanged<WebAssistantPaneInternal> onPaneChanged;
  ...
  Widget build(BuildContext context) {
    ...
    final filteredSingle = filterConversationsInternal(single, query);
    final filteredLocal = filterConversationsInternal(local, query);
    final filteredRemote = filterConversationsInternal(remote, query);
    final palette = context.palette;
    ...
    return Row(
      children: [
        Container(
          ...
          width: webAssistantSideTabRailWidthInternal,
          decoration: BoxDecoration(
          ...
          child: Column(
            children: [
              ...
              AssistantSideTabButtonInternal(
                ...
                selected: activePane == WebAssistantPaneInternal.tasks,
                tooltip: appText('任务', 'Tasks'),
                onTap: () => onPaneChanged(WebAssistantPaneInternal.tasks),
              ),
              const SizedBox(height: 4),
              AssistantSideTabButtonInternal(
                key: const Key('assistant-side-pane-tab-quick'),
                icon: Icons.dashboard_customize_outlined,
                selected: activePane == WebAssistantPaneInternal.quick,
                tooltip: appText('快捷面板', 'Quick panel'),
                onTap: () => onPaneChanged(WebAssistantPaneInternal.quick),
              ),
              const Spacer(),
              IconButton(
          ...
          Expanded(
            child: AnimatedSwitcher(
              ...
              child: KeyedSubtree(
                ...
                child: activePane == WebAssistantPaneInternal.tasks
                    ? AssistantTaskPaneInternal(
...
class AssistantQuickPaneInternal extends StatelessWidget {
  const AssistantQuickPaneInternal({
    super.key,
    required this.controller,
    required this.permissionLevel,
    required this.onPermissionChanged,
